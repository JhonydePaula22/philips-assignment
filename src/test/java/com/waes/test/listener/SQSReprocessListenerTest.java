...
class SQSReprocessListenerTest {

    @Mock
    private SupplyChainIntegration service;

    @InjectMocks
    private SQSReprocessListener listener;

    @Test
    void should_process_events_when_there_is_are_events_to_be_processed() {
        listener.receiveMessage(Event.builder().id("1").name("name").quantity(1).price(new BigDecimal("12.01")).eventType(EventTypeEnum.RETRY).action(ActionEnum.CREATE).build());
        listener.receiveMessage(Event.builder().id("1").name("name").quantity(1).price(new BigDecimal("12.01")).eventType(EventTypeEnum.RETRY).action(ActionEnum.UPDATE).build());
        listener.receiveMessage(Event.builder().id("1").name("name").quantity(1).price(new BigDecimal("12.01")).eventType(EventTypeEnum.RETRY).action(ActionEnum.DELETE).build());
